        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14">
          <div className="max-w-2xl">
            ...
            >
              ...
              <span className="text-sm font-semibold text-dark">Unsere Fachärzte</span>
            </motion.div>
            ...
            <motion.h2
              ...
              className="font-heading text-4xl md:text-5xl lg:text-[56px] font-bold text-dark leading-[1.05] tracking-tight"
            >
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button
              onClick={() => scroll("left")}
              aria-label="Previous"
              className="w-12 h-12 rounded-full bg-white shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 flex items-center justify-center text-dark hover:bg-gray-50 transition-colors active:scale-95"
            >
            ...
            <button
              ...
              aria-label="Next"
              className="w-12 h-12 rounded-full bg-dark shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex items-center justify-center text-white hover:bg-black transition-colors active:scale-95"
            >
            ...
            {doctorsData.map((doc, idx) => (
              ...
              >
                  ...
                  <div className="absolute bottom-3 left-3 right-3 bg-white rounded-[100px] p-1.5 flex items-center justify-between shadow-lg">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-[13px] font-bold text-dark leading-tight truncate">{doc.name}</p>
                        <p className="text-[11px] font-semibold text-gray-500 truncate">{doc.role}</p>
                      </div>
                    </div>
                    <a href="#" className="w-9 h-9 rounded-full bg-dark flex items-center justify-center shrink-0 hover:opacity-90 transition-opacity">
                ...
                <div className="flex flex-col flex-1 min-w-0 py-1 pr-1">
                  <div className="mb-5">
                    <p className="text-[15px] font-bold text-dark mb-1.5">Über {doc.name.split(" ")[1]}:</p>
...
const InfoCard = ({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) => (
  <div className="flex items-center gap-3 border border-gray-100 rounded-[12px] p-3 bg-[#fafafa]">
    <div className="w-[34px] h-[34px] rounded-full bg-white border border-gray-100 flex items-center justify-center shrink-0">
      <Icon size={16} strokeWidth={2.2} className="text-dark" />
    </div>
    <div className="overflow-hidden flex flex-col justify-center">
      <p className="text-[11px] text-gray-400 font-semibold mb-[2px]">{label}</p>
      <p className="text-[13px] font-bold text-dark truncate">{value}</p>
    </div>
  </div>
);
